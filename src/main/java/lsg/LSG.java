...
public class LSG {
  public LSG() {
    loopCounter = 0;
    loops = new ObjectArrayList<SimpleLoop>();
    root = new SimpleLoop();
  ...
  public void calculateNestingLevelRec(SimpleLoop loop, int depth) {
    ...
    ObjectIterator<SimpleLoop> it = loop.getChildren().iterator();
    while (it.hasNext()) {
      SimpleLoop liter = it.next();
  ...
  private ObjectArrayList<SimpleLoop> loops;
